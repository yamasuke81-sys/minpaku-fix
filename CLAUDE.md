...
## 次回やること（優先度順）
1. **[高]** 3/23清掃モーダル「確定済み」誤表示の監視 — v0310nのデバッグコード（[DEBUG-CONFIRM]ログ）で様子見中。再現したらコンソールログを確認
2. **[高]** v0308m（予約延長時の清掃モーダル修正）の動作確認 — 3/15清掃モーダルが「募集中」表示になるか、回答状況がリセットされるか確認
3. **[高]** v0308l（CI/CO時刻表示）の動作確認 — スプシのCI/COに時刻が表示されるか確認済みだが、宿泊モーダルでの時刻表示も確認
4. **[高]** v0308j（通知日付フォーマット統一）の動作確認 — 通知メッセージ内の日付がM/d(曜日)形式に統一されているか
5. **[中]** UIデバッグパネルの削除 — 通知デバッグパネル（index.html 1131行付近）と清掃モーダルDEBUGパネル（3202行付近）を全修正確認後に削除
6. **[中]** v0310nデバッグコード削除 — 確定判定の[DEBUG-CONFIRM]ログ+[DEBUG-CAL]ログ+[DEBUG-DAYCELL]ログ。問題が再現しなければ削除
7. **[中]** 「募集」シートの大量重複エントリのクリーンアップ — ログで200行のエントリが確認されており、同一行番号の重複が多数ある
...
- **checkin-app/Code.gs行数**: ~762行
- **checkin-app/checkin.html行数**: ~918行
- **進行中タスク**: チェックインアプリの改善
