...
│   ├── Code.gs                  # チェックリストアプリ GAS コード（~2888行）
│   ├── checklist.html           # チェックリストアプリ UI（~4718行）
...
```
00e40bb docs: 引き継ぎルールを強化（質問→返答待ち→作業開始、ルール引き継ぎ必須化）
53abf0f docs: 引き継ぎ資料を最新状態に更新（50+コミット分の変更を反映）
4308e4c fix: スタッフ選択の「粘着ユニオン問題」を修正
f9e50f5 fix: スタッフ選択の複数端末マージ対応 & 同期の競合状態修正
2568864 fix: 清掃完了日時の表示フォーマットを修正
3cb4775 feat: 写真保管期間設定UI追加 & コインランドリー位置変更
1749c45 feat: チェックリストapp多機能強化
12aa4a0 UI: クリーニング状況の見出しをスタッフ確定済みと同じバナー形式に変更
fc73fd4 fix: 撮影タブのカテゴリ順をチェックリストタブに合わせる
23a8948 fix: チェック解除(uncheck)処理をdeleteRowからバッチ方式に変更
e15f4e9 fix: スタッフ選択済み日付でドロップダウンが消えるバグを修正
c3c5084 fix: deploy-all.jsでチェックリストアプリのバージョンを明示作成
861c78d fix: スタッフ入力欄にインラインスタイル追加（CSS破損対策）
86929f5 revert: オーナー専用編集制限を撤廃（不具合多発のため）
8c5285b fix: GAS CSSサニタイザー対策でbodyセレクタを#contentAreaに変更
648659c チェックリスト設定タブにメモ写真フォルダの設定を追加
f9c327e deploy-all.js: デプロイID1つ前提にシンプル化
91244fe スタッフ用URLをオーナー用URL+?staff=1から自動生成するように変更
51ffef8 feat: 実際のアプリスクショを使ったスタッフ操作マニュアル生成ツール
9d5a184 refactor: 仕事内容タブのUIを改善 - 折りたたみ見出し+インライン金額入力
fa63079 feat: 古いデータの自動クリーンアップ機能を追加
56c8a9a feat: スタッフ選択を複数端末間で同期する機能を追加
54fc0bd fix: 複数スマホ同時利用時のチェックリスト同期問題を修正
f76b93b feat: メモを登録ボタン方式に変更＋削除機能を追加
2e359df feat: 特記事項メモに写真添付機能を追加
6497c56 feat: 中カテゴリを大カテゴリ（ルートレベル）に移動できるようにする
f80fa45 fix: 要補充アイテムの復活問題を3方向から修正
```
...
| 清掃ステータスマーク修正 | スタッフ確定後にカレンダーのステータスドットが即座に緑に変わるよう修正 | **完了（要テスト）** |
| 清掃担当UI改善 | 編集ボタンをラベル直下に配置、バナー形式の見出しに変更 | **完了** |
| スタッフ選択済みバグ修正 | ドロップダウンが消えるバグを修正 | **完了** |
| スタッフ入力欄つぶれ対策 | インラインスタイルでCSS破損を防止 | **完了** |
| スタッフ用URL改善 | オーナー用URL+?staff=1で自動生成、Brave対応 | **完了** |

### チェックリストアプリ (`checklist-app/`)

| 項目 | 内容 | 状態 |
|------|------|------|
| スタッフ選択の複数端末同期 | deviceId別管理、粘着ユニオン問題修正 | **完了（要テスト）** |
| メモ機能全面改修 | 登録ボタン方式＋削除機能＋写真添付 | **完了** |
| 要補充リスト修正 | 5段階のバグ修正（復活問題、カテゴリ表示等） | **完了** |
| カテゴリ移動 | 中カテゴリをルートレベルに昇格可能 | **完了** |
| 自動クリーンアップ | 古い写真・記録・メモの自動削除 | **完了** |
| 仕事内容タブUI改善 | 折りたたみ見出し＋インライン金額入力 | **完了** |
| 写真保管期間設定UI | 設定タブに追加 | **完了** |
| カテゴリ見出し差別化 | 大=濃紺、中=薄青、小=オレンジ、細=紫 | **完了** |
| 全展開/全折りたたみ | カテゴリ見出しにボタン追加 | **完了** |
| カテゴリ名称変更 | ✎ ボタンで変更可能 | **完了** |
| カテゴリ削除 | ✕ ボタンで削除（中身削除 or 親に移動） | **完了** |
| 要補充タブ「表示」ボタン | 大カテゴリ名バッジ＋該当項目へスクロール | **完了** |
| タブナビ上部配置 | sticky対応のボタン形式タブ | **完了** |
| 漏れチェック統合 | 清掃完了ボタンに統合 | **完了** |
| メモ→登録ボタン方式 | 自動保存を廃止し登録ボタンに変更 | **完了** |
| 項目名タッチ | テキストタッチでもチェック切替 | **完了** |
| 見本写真サムネイル | 28x28pxのサムネイル表示 | **完了** |
| チェック解除バッチ方式 | deleteRowからバッチ方式に変更 | **完了** |
| 撮影タブカテゴリ順 | チェックリストタブに合わせる | **完了** |
| 清掃完了日時フォーマット | 表示フォーマット修正 | **完了** |
| 設定タブ | メモ写真フォルダ設定を追加 | **完了** |
| GAS CSSサニタイザー対策 | bodyセレクタを#contentAreaに変更 | **完了** |
...
2. **清掃ステータスマーク修正の確認**
   ...
   - **関連GAS関数**: `updateCleaningStaff()`, `confirmRecruitment()`, `selectStaffForRecruitment()`, `getRecruitmentStatusMap()`, `saveRecruitmentDetail()`
   - **関連フロントエンド**: `buildCalendarEvents()`, `loadAndRender()`, `applyConfirmOptimistic()`
