...
- **デバッグ表示（フロントエンドのデバッグ欄・console.log等）を勝手に削除すること** — 修正が完全に動作確認できた後、ユーザーに削除を提案し、許可を得てから削除する
...
1. `~/.claude/CLAUDE-TEMPLATE.md` を確認する（全プロジェクト共通のテンプレート置き場）
2. ある場合は `CLAUDE-TEMPLATE.md` をプロジェクトルートに `CLAUDE.md` としてコピーし、プレースホルダーをプロジェクトに合わせて埋める
3. ない場合はユーザーに「~/.claude/CLAUDE-TEMPLATE.mdが見つかりません。テンプレートの場所を教えてください」と確認する
