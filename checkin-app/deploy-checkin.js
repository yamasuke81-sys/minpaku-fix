...
async function main() {
  ...
  if (!claspConfig.scriptId || claspConfig.scriptId === 'YOUR_CHECKIN_SCRIPT_ID_HERE') {
  ...
  const savedCheckinId = (config.checkinDeploymentId || '').trim();
  ...
  if (savedCheckinId) {
    console.log('   保存済みデプロイIDで更新: ' + savedCheckinId.substring(0, 30) + '...');
    deployResult = runCapture(`${clasp} deploy --deploymentId "${savedCheckinId}" --description "チェックイン ${today}"`);
    if (deployResult.success) {
      deployId = savedCheckinId;
  ...
  if (!deployId) {
    ...
    if (existingIds.length > 0) {
      ...
      deployResult = runCapture(`${clasp} deploy --deploymentId "${foundId}" --description "チェックイン ${today}"`);
      if (deployResult.success) {
        deployId = foundId;
        deployUrl = 'https://script.google.com/macros/s/' + deployId + '/exec';
      }
  ...
  if (!deployId) {
    ...
    deployResult = runCapture(`${clasp} deploy --description "チェックイン ${today}"`);
    if (!deployResult.success) {
  ...
  if (deployId && deployId !== savedCheckinId) {
    config.checkinDeploymentId = deployId;
    try {
      fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n', 'utf8');
      console.log('   deploy-config.json にチェックインデプロイIDを保存しました。');
    } catch (e) {
