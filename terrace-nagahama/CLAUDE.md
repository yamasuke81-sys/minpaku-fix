...
| 施設紹介 | 完了 | ギャラリー・特徴紹介・間取り・設備一覧 |
| 料金・予約 | 完了 | 料金表・外部予約サービス誘導（URL未設定）・キャンセルポリシー |
| アクセス | 未着手 | - |
| FAQ | 未着手 | - |
| お問い合わせ | 未着手 | - |
...
| 料金計算ロジック | 不要 | 外部予約サービスを利用するため不要 |
| 予約フォーム | 不要 | 外部予約サービスへ誘導 |
| メール通知 | 未着手 | - |
| iCal連携 | 未着手 | - |
...
| 予約サービスURL未設定 | 高 | pricing.blade.php の `$bookingServices` にURL設定が必要 |

## 9. 次回セッションでやるべきことリスト

1. アクセスページの実装（Google Map埋め込み）
2. FAQページの実装
3. お問い合わせページの実装（コンタクトフォーム + メール送信）
4. 予約サービスURL設定（Airbnb / 楽天トラベル等が決まり次第、pricing.blade.phpの`$bookingServices`を更新）
5. 写真素材の差し替え（プレースホルダー → 実際の施設写真）
6. 管理画面の拡充（必要に応じて）
